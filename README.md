...
4. **Slide-Level Detail:** Read `lecture_readers/pdf/CS8165_complete_lecture_readers.pdf` or the individual files in `lecture_readers/markdown/` when you need professor-style slide-by-slide explanation.
5. **Raw Lecture Text:** Read the matching original text chunk in `course_text_parts/03_lectures/` after the easier versions, so the original wording becomes less intimidating.
6. **Assignment Reading:** Read the matching assignment in `course_text_parts/04_assignments/`, then browse `assignment_sources/` and `assignment_zips/` where available.
7. **Checklist Gate:** Read and use `exam_materials/01_mastery_checklists.md`. Do not move on from a lecture until you can identify the topic, its pipeline role, its main terms, and its common trap.
8. **Cloze Practice:** Copy passages from `exam_materials/02_cloze_generator_inputs.md` or `practice_pack/cloze_source_texts.md` into a cloze generator.
9. **Matching Practice:** Use `exam_materials/03_matching_pairs.tsv`, `exam_materials/03_matching_tasks.md`, and `practice_pack/matching_pairs.tsv` to drill terms, definitions, chapters, and roles.
10. **Ordering Practice:** Use `exam_materials/05_sequencing_and_pipeline_tasks.md` and `practice_pack/sequencing_tasks.md` to learn process order, especially for pipeline, projection, clipping, rasterization, and visibility.
11. **Closed-Format Drills:** Use `exam_materials/04_closed_format_drills.md`, `practice_pack/multiple_choice.md`, and `overprep_pack/mock_exams/`. Prefer multiple choice, matching, completion, ordering, labeling, and debugging tasks over broad open essays.
12. **Math, Algorithms, And Diagrams:** Use `study_pack/formulas_and_derivations.md`, `practice_pack/math_algorithm_drills.md`, `exam_materials/06_diagram_label_tasks.md`, and `practice_pack/diagram_graphics_prompts.md`.
13. **OpenGL And Software:** Use `exam_materials/07_opengl_debugging_drills.md`, `practice_pack/opengl_software_drills.md`, `course_text_parts/05_opengl_starter_project.txt`, and the software labs in `reader_pack/interactive_workbook.html`.
14. **Non-Boring Repetition:** Use `practice_pack/repetition_variants/round_01`, then `round_02`, then `round_03`. These repeat the same concepts with changed wording and different task forms.
15. **Mistake Repair:** Log every wrong answer in `overprep_pack/mistake_log.md`, then turn it into repair work with `exam_materials/10_mistake_log_repair_drills.md`.
16. **Final Simulation:** Use `exam_materials/09_final_mixed_closed_exam.md`, `study_pack/exam_drill.md`, and the mock exams in `overprep_pack/mock_exams/`.
