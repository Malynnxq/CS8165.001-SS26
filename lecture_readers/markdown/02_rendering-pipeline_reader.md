...
Professor-style explanation: The slide 'Object-based Rendering' explains object-based rendering as a conversion chain. A model supplies geometric objects such as vertices or primitives. The geometry stage transforms and assembles those objects. Rasterization turns primitives into fragments, and fragment operations decide which fragments update pixels in the framebuffer. On the slide, the concrete items are: - Image generation done through rasterization / - Scene objects formed by 3D models (primitives) / are transformed through successive coordinate systems / - Primitives are rasterized into fragments, which are pixel predecessors / - A subset of the fragments becomes pixels. The essential relation is that every stage changes the representation of the same scene information.
...
Professor-style explanation: The slide '2.1 Rendering Process' explains object-based rendering as a conversion chain. A model supplies geometric objects such as vertices or primitives. The geometry stage transforms and assembles those objects. Rasterization turns primitives into fragments, and fragment operations decide which fragments update pixels in the framebuffer. On the slide, the concrete items are: 2.2 OpenGL Overview / 2.3 OpenGL Rendering / 2.4 OpenGL Rendering Pipeline / 2.5 Fragment Tests and Operations / 2.6 Framebuffer. The essential relation is that every stage changes the representation of the same scene information.
...
Professor-style explanation: The slide '2.1 Rendering Process' explains object-based rendering as a conversion chain. A model supplies geometric objects such as vertices or primitives. The geometry stage transforms and assembles those objects. Rasterization turns primitives into fragments, and fragment operations decide which fragments update pixels in the framebuffer. On the slide, the concrete items are: From 3D models over fragments to pixels. The essential relation is that every stage changes the representation of the same scene information.
...
Professor-style explanation: The slide 'Geometry-based Rendering 1/2' explains object-based rendering as a conversion chain. A model supplies geometric objects such as vertices or primitives. The geometry stage transforms and assembles those objects. Rasterization turns primitives into fragments, and fragment operations decide which fragments update pixels in the framebuffer. On the slide, the concrete items are: CPU Application GPU Rendering Display Hardware / Rendering / 3D Model Framebuffer / Process / Video. The essential relation is that every stage changes the representation of the same scene information.
...
Professor-style explanation: The slide 'Geometry-based Rendering 2/2' explains object-based rendering as a conversion chain. A model supplies geometric objects such as vertices or primitives. The geometry stage transforms and assembles those objects. Rasterization turns primitives into fragments, and fragment operations decide which fragments update pixels in the framebuffer. On the slide, the concrete items are: - The rendering process is realized through the rendering pipeline / - Renders 3D models to 2D images / - Conceptually, the rendering pipeline consists of two GPU stages / - Geometry stage – processes geometric primitives / - Rasterization stage – processes fragments and pixels. The essential relation is that every stage changes the representation of the same scene information.
...
Professor-style explanation: The slide 'Rasterization Stage' explains the conversion from continuous geometry to a discrete sample grid. A mathematical line, triangle, or region is tested against pixel/sample positions. Covered samples become fragments, and attributes such as depth, color, normals, or texture coordinates can be interpolated across the primitive. On the slide, the concrete items are: - Conversion of primitives into fragments (rasterization) / - Discretization takes place in relation to a target grid / - Rasterization algorithms optimized for certain primitives / (e.g., lines, triangles) / - After rasterization per-fragment tests are performed. Rasterization creates fragment candidates; later tests decide whether those candidates become visible pixel updates.
...
Professor-style explanation: The slide '2.2 OpenGL Overview' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: Library for real-time computer graphics. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'OpenGL History' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Unified graphics software interface was missing / as knowledge channeled into proprietary software packages / - HP's starbase / - SGI's Graphics Library (GL) / - Some standardization efforts existed. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'OpenGL' explains object-based rendering as a conversion chain. A model supplies geometric objects such as vertices or primitives. The geometry stage transforms and assembles those objects. Rasterization turns primitives into fragments, and fragment operations decide which fragments update pixels in the framebuffer. On the slide, the concrete items are: - OpenGL (Open Graphics Library) / - Platform independent 2D and 3D graphics API / - API specification is provided by the Khronos Group / - Technical implementation / - Rendering pipeline consisting of static and programmable pipeline stages. The essential relation is that every stage changes the representation of the same scene information.
...
Professor-style explanation: The slide 'OpenGL vs. Vulkan' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - How about Vulkan? / - More modern than OpenGL / - Designed for high performance and low-level GPU control / - Better suited for complex, large-scale rendering engines / - Vulkan also harder to learn and requires more setup/code. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'OpenGL Libraries' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Different tastes of OpenGL / - OpenGL / - OpenGL ES - limited version for embedded systems / - WebGL - Javascript connection for Web Development / (functionality similar to OpenGL ES). The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'OpenGL Functionality' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Functional API / - Objects, scenes and images are described by a sequence of commands / (functional approach, low-level) / - Contrast: scene or object description (declarative approach, high-level) / - OpenGL context. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'OpenGL Context' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Context is created and modified by the application, / and contains all the state variables of the OpenGL engine / - Any number of contexts per application possible, / but at most one active context per application / - Context is generally tied to an output window. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'OpenGL Rendering Process' explains object-based rendering as a conversion chain. A model supplies geometric objects such as vertices or primitives. The geometry stage transforms and assembles those objects. Rasterization turns primitives into fragments, and fragment operations decide which fragments update pixels in the framebuffer. On the slide, the concrete items are: - Initiation from the developer's perspective / - Creation of the output window / - Creation of the context / - Specification of the scene geometry / - Uploading of the scene geometry to the GPU. The essential relation is that every stage changes the representation of the same scene information.
...
Professor-style explanation: The slide 'Window Creation with GLFW 1/2' introduces a concrete graphics object, operation, or relation. The named terms describe input data, a processing step, and an output used elsewhere in the rendering workflow. On the slide, the concrete items are: #include <GLFW/glfw3.h> / #include <stdlib.h> / #include <stdio.h> / static void error_callback(int error, const char* description) { / fputs(description, stderr). The object-level relation is therefore input, operation, output, and the later graphics stage that consumes the output.
...
Professor-style explanation: The slide 'Window Creation with GLFW 2/2' introduces a concrete graphics object, operation, or relation. The named terms describe input data, a processing step, and an output used elsewhere in the rendering workflow. On the slide, the concrete items are: int main(void){ / GLFWwindow* window; / glfwSetErrorCallback(error_callback); / if (!glfwInit()) exit(EXIT_FAILURE); / window = glfwCreateWindow(640, 480, "Simple example", NULL, NULL). The object-level relation is therefore input, operation, output, and the later graphics stage that consumes the output.
...
Professor-style explanation: The slide 'OpenGL Naming Conventions' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Function-Prefix "gl" / - glClear, ... / - Constant-Prefix "GL" / - GL_POLYGON, GL_LINES, ... / - Parameter types are explicitly encoded in function names. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide '2.3 OpenGL Rendering' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: Transfer of 3D models to the GPU. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'OpenGL Rendering (until V2.1)' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Rendering is done through Immediate Mode / void renderScene() { / glColor3f(0.0f, 1.0f, 0.0f); // drawing color becomes green / glBegin(GL_POLYGON); // begin primitive / glVertex2f(-0.5f,-0.5f); // 1. vertex. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'OpenGL Rendering (since V3.0)' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Using Vertex Array Objects (VAOs) / - Vertex specification in CPU memory / - Vertex data transfer from CPU to GPU / - Memory layout description of the vertex data / - Number of vertices, number of components (x, y, z, ...). The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide '1. Vertex Specification' explains the conversion from continuous geometry to a discrete sample grid. A mathematical line, triangle, or region is tested against pixel/sample positions. Covered samples become fragments, and attributes such as depth, color, normals, or texture coordinates can be interpolated across the primitive. On the slide, the concrete items are: - Vertex data is specified as a C array / // vertices of two triangles / GLfloat vertices[6][2] = { // 6 vertices with two components each (x,y) / {0.0, 1.0}, {1.0, 1.0}, {1.0, 0.0}, // 1. triangle / {0.5, 0.5}, {1.0, 0.3}, {0.7, 1.0} // 2. triangle. Rasterization creates fragment candidates; later tests decide whether those candidates become visible pixel updates.
...
Professor-style explanation: The slide '2. Vertex Data Transfer' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - VBO allocation and data transfer via / - glBufferData(GL_ARRAY_BUFFER, size, data, GL_STATIC_DRAW); / - data – address in main memory (i.e., vertices or &vertices[0]) / - size – size of data in number of bytes / (i.e. sizeof(vertices) or numVertices*sizeof(GLfloat)*2). The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide '4. VAO Drawing 1/2' explains the conversion from continuous geometry to a discrete sample grid. A mathematical line, triangle, or region is tested against pixel/sample positions. Covered samples become fragments, and attributes such as depth, color, normals, or texture coordinates can be interpolated across the primitive. On the slide, the concrete items are: - Drawing is done with the command / - glDrawArrays(mode, first, count); / - mode – specifies the primitive type (e.g., GL_TRIANGLES or GL_LINES) / - From all vertices, the next count vertices are drawn from index first / - Actual vertex data no longer needs to be specified. Rasterization creates fragment candidates; later tests decide whether those candidates become visible pixel updates.
...
Professor-style explanation: The slide '4. VAO Drawing 2/2' explains the conversion from continuous geometry to a discrete sample grid. A mathematical line, triangle, or region is tested against pixel/sample positions. Covered samples become fragments, and attributes such as depth, color, normals, or texture coordinates can be interpolated across the primitive. On the slide, the concrete items are: - There is a small number of primitive types / - Points / - Lines, Line Strips, Line Loops / - Triangles, Triangle Strips, Triangle Fans, Patches / v2 v2 v2. Rasterization creates fragment candidates; later tests decide whether those candidates become visible pixel updates.
...
Professor-style explanation: The slide '5. GPU Memory Release' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - After usage VAO and VBO should be deleted from GPU memory / - glDeleteBuffers(…); / - glDeleteVertexArrays(…). The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Rendering Initialization 1/2' explains the conversion from continuous geometry to a discrete sample grid. A mathematical line, triangle, or region is tested against pixel/sample positions. Covered samples become fragments, and attributes such as depth, color, normals, or texture coordinates can be interpolated across the primitive. On the slide, the concrete items are: static void init_render_scene(void) { / static const GLfloat vertices[6][2] = { / {0.0f, 1.0f}, {1.0f, 1.0f}, {1.0f, 0.0f}, /* 1st triangle */ / {0.5f, 0.5f}, {1.0f, 0.3f}, {0.7f, 1.0f} /* 2nd triangle */ / static const char* vs_src =. Rasterization creates fragment candidates; later tests decide whether those candidates become visible pixel updates.
...
Professor-style explanation: The slide 'Rendering Initialization 2/2' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: program = create_program(vs_src, fs_src); / glGenVertexArrays(1, &vao); / glGenBuffers(1, &vbo); / glBindVertexArray(vao); / glBindBuffer(GL_ARRAY_BUFFER, vbo). The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Rendering' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: static void render_scene(void) { / glClear(GL_COLOR_BUFFER_BIT); / glUseProgram(program); / glBindVertexArray(vao); / glDrawArrays(GL_TRIANGLES, 0, 6). The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Additional Attributes' explains how geometric objects change coordinate systems. A point, vector, normal, or local coordinate frame is multiplied by a transformation matrix or affected by an affine operation. Translations move positions, rotations change orientation, scaling changes size, and composed matrices combine several such effects. On the slide, the concrete items are: - Additional attributes are necessary for drawing / - Color, normals, texture coordinates, ... / - There are two ways to assign these attributes to the vertex data / - Attributes are bound to the same VAO in separate VBO / - Attributes are stored interleaved with vertex data,. The object-level relation is input coordinate space, transformation object, and output coordinate space.
...
Professor-style explanation: The slide '2.4 OpenGL Rendering Pipeline' explains object-based rendering as a conversion chain. A model supplies geometric objects such as vertices or primitives. The geometry stage transforms and assembles those objects. Rasterization turns primitives into fragments, and fragment operations decide which fragments update pixels in the framebuffer. On the slide, the concrete items are: Programmable and static stages. The essential relation is that every stage changes the representation of the same scene information.
...
Professor-style explanation: The slide 'The Rendering Pipeline 1/2' explains object-based rendering as a conversion chain. A model supplies geometric objects such as vertices or primitives. The geometry stage transforms and assembles those objects. Rasterization turns primitives into fragments, and fragment operations decide which fragments update pixels in the framebuffer. On the slide, the concrete items are: Scene Description Raster Image / Geometry Fragment / Rasterization / Processing Processing / Vertices Primitives Fragments Pixels. The essential relation is that every stage changes the representation of the same scene information.
...
Professor-style explanation: The slide 'The Rendering Pipeline 2/2' explains object-based rendering as a conversion chain. A model supplies geometric objects such as vertices or primitives. The geometry stage transforms and assembles those objects. Rasterization turns primitives into fragments, and fragment operations decide which fragments update pixels in the framebuffer. On the slide, the concrete items are: - Sequence of fixed and programmable stages Programmable / Vertex Processing / - Process is influenced by context and scene objects / Context Fixed Function / - Programmable levels are programmed. The essential relation is that every stage changes the representation of the same scene information.
...
Professor-style explanation: The slide 'Shader Programming – History 1/2' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - There is special graphics hardware (dedicated) / and general graphics hardware (general purpose) / - Special graphics hardware / - Volume Rendering: VolumePro / - Game consoles: Custom NVIDIA chips in XBox. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Shader Programming – History 2/2' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Various vendor-dependent extensions are introduced / to provide extensibility for general graphics hardware / - Texture shaders and register combiners allowed modification / through the application of textures / - Applicability is very limited by the possibilities of graphics hardware. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Assembler Shader' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - First OpenGL extensions support shader programming in assembler code / !! ARBvp1.0 / ATTRIB iPos = vertex.position ; / ATTRIB iNormal = vertex.normal ; / . The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'High-Level Shading Languages 1/2' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Assembler code is hard to read and hard to maintain / - High-level languages f or shading are specified in C-like syntax / - Source code is translated by the compiler into appropriate ASM statements / - Using shaders after initial binding automatically when rendering geometry / - Interface to OpenGL by passing appropriate variables or changing the OpenGL. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'High-Level Shading Languages 2/2' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Existing high level languages f or shader development / - CG - C for Graphics by NVIDIA / - CG was introduced in 2003 by NVIDIA as a platform independent solution / (in practice rather semi-platform independent) / - HLSL - High Level Shading Language from Microsoft. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Shader Types' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Pipeline concept has been kept in place / and shaders are responsible for different stages / - First, geometry level programming was done by vertex shaders / - Vertex shaders allow modifiability of vertex transformations and lighting / - After that, there was an extension for image-based algorithms by fragment. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Vertex Shader Concepts' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Vertex shaders enable the programmable modification of processed geometry / - Possibilities of a vertex shader / Programmable / - Geometric transformation of vertices and normals / Vertex Processing. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Vertex Shader Examples 1/2' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: No object-level text was extracted from this page; the page is primarily title, image, diagram, or layout content. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Vertex Shader Examples 2/2' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Morphing / - Shadow volume calculation for shadow effects / [ID Software‘s Doom III]. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Vertex Shader Coordinate Systems 2/2' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: Local Space World Space View Space Clip Space Screen Space / Viewport / 𝑀 𝑀 𝑀 / 𝑚𝑜𝑑𝑒𝑙 𝑣𝑖𝑒𝑤 𝑝𝑟𝑜𝑗𝑒𝑐𝑡 Transform. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Local Space' explains how geometric objects change coordinate systems. A point, vector, normal, or local coordinate frame is multiplied by a transformation matrix or affected by an affine operation. Translations move positions, rotations change orientation, scaling changes size, and composed matrices combine several such effects. On the slide, the concrete items are: - Coordinates of your object relative to its local origin / - 3D vertex positions. The object-level relation is input coordinate space, transformation object, and output coordinate space.
...
Professor-style explanation: The slide 'World Space' explains how geometric objects change coordinate systems. A point, vector, normal, or local coordinate frame is multiplied by a transformation matrix or affected by an affine operation. Translations move positions, rotations change orientation, scaling changes size, and composed matrices combine several such effects. On the slide, the concrete items are: - Coordinates in respect of a larger world / - Relative to some global origin of the world / - Together with many other objects. The object-level relation is input coordinate space, transformation object, and output coordinate space.
...
Professor-style explanation: The slide 'View Space' explains how geometric objects change coordinate systems. A point, vector, normal, or local coordinate frame is multiplied by a transformation matrix or affected by an affine operation. Translations move positions, rotations change orientation, scaling changes size, and composed matrices combine several such effects. On the slide, the concrete items are: - Coordinates as seen from the camera. The object-level relation is input coordinate space, transformation object, and output coordinate space.
...
Professor-style explanation: The slide 'Clip Space' explains how geometric objects change coordinate systems. A point, vector, normal, or local coordinate frame is multiplied by a transformation matrix or affected by an affine operation. Translations move positions, rotations change orientation, scaling changes size, and composed matrices combine several such effects. On the slide, the concrete items are: - Clip coordinates range from -1 to 1 / - Projection (Orthographic, Perspective). The object-level relation is input coordinate space, transformation object, and output coordinate space.
...
Professor-style explanation: The slide 'Screen Space' explains how geometric objects change coordinate systems. A point, vector, normal, or local coordinate frame is multiplied by a transformation matrix or affected by an affine operation. Translations move positions, rotations change orientation, scaling changes size, and composed matrices combine several such effects. On the slide, the concrete items are: - Transform to image coordinates with glViewport / - Results are sent to the rasterizer. The object-level relation is input coordinate space, transformation object, and output coordinate space.
...
Professor-style explanation: The slide 'Coordinate Systems' explains how geometric objects change coordinate systems. A point, vector, normal, or local coordinate frame is multiplied by a transformation matrix or affected by an affine operation. Translations move positions, rotations change orientation, scaling changes size, and composed matrices combine several such effects. On the slide, the concrete items are: Local Space World Space View Space Clip Space Screen Space / Viewport / 𝑀 𝑀 𝑀 / 𝑚𝑜𝑑𝑒𝑙 𝑣𝑖𝑒𝑤 𝑝𝑟𝑜𝑗𝑒𝑐𝑡 Transform. The object-level relation is input coordinate space, transformation object, and output coordinate space.
...
Professor-style explanation: The slide 'Vertex Shader Variables' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Input types / - Uniforms: read only in VS (& FS) // Intrinsic vertex attributes / out gl_PerVertex { / - For example: light position or light color / vec4 gl_Position. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Fixed Function Vertex Postprocessing' explains how camera geometry maps 3D positions toward a 2D image. The projection matrix maps view-space objects into clip space. Perspective projection uses the homogeneous component so that the perspective divide makes distant objects appear smaller; orthographic projection preserves apparent size. On the slide, the concrete items are: - Clipping / - Discard invisible geometries / Programmable / - Perspective Division / Vertex Processing. The object-level chain is camera/view volume, projection matrix, clip coordinates, normalized device coordinates, and viewport coordinates.
...
Professor-style explanation: The slide 'Fixed Function Rasterization' explains how geometric objects change coordinate systems. A point, vector, normal, or local coordinate frame is multiplied by a transformation matrix or affected by an affine operation. Translations move positions, rotations change orientation, scaling changes size, and composed matrices combine several such effects. On the slide, the concrete items are: - Geometry is converted to raster coordinates / Programmable / Vertex Processing / Fixed Function / Primitives Fragments Vertex Postprocessing. The object-level relation is input coordinate space, transformation object, and output coordinate space.
...
Professor-style explanation: The slide 'Fragment Shader Concept 1/2' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Fragment shaders set the color of a pixel / - Possibilities of a fragment shader / Programmable / - Glare of textures / Vertex Processing. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Fragment Shader Concept 2/2' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Fragment shaders are executed for fragments (= pixel predecessors) / - Fragment shaders calculate the color and depth value of the current fragment / - No information about adjacent fragments available / - The number of fragments can be changed / - Fragment shaders can not generate fragments. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Shader Example' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: layout (std140) uniform Matrices { Vertex Shader / mat4 projModelViewMatrix; / mat3 normalMatrix; /** Fragment Shader / }; * Set fragment color to red. / in vec3 position. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'GLSL Shader Functions' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Trigonometry functions / - radians, degrees, sin, cos, tan, asin, acos, atan / - Power calculations / - pow, exp2, log2, sqrt, inversesqrt / - General. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Shader Usage' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - One or multiple shaders are attached to a shader program / Vertex Shader / glCreateShader glCreateProgram / Fragment Shader / glShaderSource glAttachShader glCreateShader. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Binding Shaders' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: v = glCreateShader(GL_VERTEX_SHADER); // vertex shader / char* vs = readTextFile(”vertex-shader.vert"); / const char* vv = vs; / glShaderSource(v, 1, &vv, NULL); / free(vs). The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Setting Uniforms' introduces a concrete graphics object, operation, or relation. The named terms describe input data, a processing step, and an output used elsewhere in the rendering workflow. On the slide, the concrete items are: - Uniforms are used to communicate between CPU and GPU / - Data is bound to uniform locations / - Must be updated after each linking process / GLint loc0 = glGetUniformLocation(p, ”isoValue_"); / glUniform1f(loc0, 0.45f). The object-level relation is therefore input, operation, output, and the later graphics stage that consumes the output.
...
Professor-style explanation: The slide 'Shader Performance' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Guidelines for efficient shaders / - Since fragment shaders are executed for a large number of fragments, / they should contain as few instructions as possible / - There should be as little as possible a change of the active shaders, / because they have their own state and, accordingly, the pipeline may need. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Shader Development Environments 1/2' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - FX Composer, NVPerfHUD (NVIDIA) / - HLSL Shader IDE with performance analytics / www.developer.nvidia.com/page/tools.html / - RenderMonkey (ATI) / - HLSL, GLSL Shader IDE with performance analytics. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Shader Development Environments 2/2' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Shdr online GLSL editor: https://shdr.bkcore.com/. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide '2.5 Fragment Tests and Operations' explains the conversion from continuous geometry to a discrete sample grid. A mathematical line, triangle, or region is tested against pixel/sample positions. Covered samples become fragments, and attributes such as depth, color, normals, or texture coordinates can be interpolated across the primitive. On the slide, the concrete items are: Towards the finishing line to the pixel. Rasterization creates fragment candidates; later tests decide whether those candidates become visible pixel updates.
...
Professor-style explanation: The slide 'Fragment Tests and Operations 1/2' introduces a concrete graphics object, operation, or relation. The named terms describe input data, a processing step, and an output used elsewhere in the rendering workflow. On the slide, the concrete items are: - After fragments have been generated, they must go through the per-fragment / tests and operations / Programmable / - Only if all these tests are successful, / Vertex Processing. The object-level relation is therefore input, operation, output, and the later graphics stage that consumes the output.
...
Professor-style explanation: The slide 'Fragment Tests and Operations 2/2' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: Fragment Pixel Ownership Scissor Alpha / Data Test Test Test / Depth Stencil / Framebuffer Dithering Blending / Test Test. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Alpha Test' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - If framebuffer is an RGBA buffer, the alpha test can be performed / - The test must first be activated / glEnable(GL_ALPHA_TEST); / - The comparison function indicates which fragments survive the test / glAlphaFunc(func, value). The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Stencil Test' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Stencil Buffer / - Is a frame buffer component / - Is (indirectly) filled by drawing operations / - No special stencil drawing operations / - Stencil operation. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Depth Test' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - If the framebuffer has a depth buffer, the depth test can be performed / - The test must first be activated / glEnable(GL_DEPTH_TEST); / - The comparison is made with the depth value already in the buffer / - The comparison function indicates which fragments survive the test. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Image Composition and Mixture' introduces a concrete graphics object, operation, or relation. The named terms describe input data, a processing step, and an output used elsewhere in the rendering workflow. On the slide, the concrete items are: - Alpha values o f the pixels / - control the combination of pixel values / - are specified as color components of color pixels / - 𝑃 = [𝑅, 𝐺, 𝐵, 𝐴], 𝑤𝑖𝑡ℎ 𝑅, 𝐺, 𝐵, 𝐴 ∈ [0,1] / - Applications are manifold. The object-level relation is therefore input, operation, output, and the later graphics stage that consumes the output.
...
Professor-style explanation: The slide 'Blending Application' introduces a concrete graphics object, operation, or relation. The named terms describe input data, a processing step, and an output used elsewhere in the rendering workflow. On the slide, the concrete items are: - Example application for transparency: drawing with semi-transparent brush. The object-level relation is therefore input, operation, output, and the later graphics stage that consumes the output.
...
Professor-style explanation: The slide 'Blending Process 1/2' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Blending is an operation called per fragment / - Fragments are modified after rasterization, / but before writing to the framebuffer with a blending function / - Combination of a source fragment to be written with the destination pixel / in the frame buffer by using a blending function. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Blending Process 2/2' introduces a concrete graphics object, operation, or relation. The named terms describe input data, a processing step, and an output used elsewhere in the rendering workflow. On the slide, the concrete items are: - Source factor: 𝐹 / - Destination factor: 𝐹 / - Source fragment: [𝑃 , 𝑃 , 𝑃 , 𝑃 ] / 𝑠,𝑟 𝑠,𝑔 𝑠,𝑏 𝑠,𝑎 / - Destination pixel: [𝑃 , 𝑃 , 𝑃 , 𝑃 ]. The object-level relation is therefore input, operation, output, and the later graphics stage that consumes the output.
...
Professor-style explanation: The slide 'Applying Blending' introduces a concrete graphics object, operation, or relation. The named terms describe input data, a processing step, and an output used elsewhere in the rendering workflow. On the slide, the concrete items are: - Switch on (or switch off) blending / - glEnable(GL_BLEND); / - glDisable(GL_BLEND); / - Defining Blending Factors / - glBlendFunc(srcFac, destFac). The object-level relation is therefore input, operation, output, and the later graphics stage that consumes the output.
...
Professor-style explanation: The slide 'Blending Examples' introduces a concrete graphics object, operation, or relation. The named terms describe input data, a processing step, and an output used elsewhere in the rendering workflow. On the slide, the concrete items are: - Example 1: Copy source pixels and overwrite destination pixels / - glBlendFunc(GL_ONE, GL_ZERO); / - 𝑃 = [𝑃 , 𝑃 , 𝑃 , 𝑃 ] / 𝑏𝑙𝑒𝑛𝑑 𝑠,𝑟 𝑠,𝑔 𝑠,𝑏 𝑠,𝑎 / - Example 2: Alpha-based mixing of source pixels and destination pixels. The object-level relation is therefore input, operation, output, and the later graphics stage that consumes the output.
...
Professor-style explanation: The slide '2.6 Framebuffer' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: Contains everything that is (in)visible on the screen. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Viewport' explains how camera geometry maps 3D positions toward a 2D image. The projection matrix maps view-space objects into clip space. Perspective projection uses the homogeneous component so that the perspective divide makes distant objects appear smaller; orthographic projection preserves apparent size. On the slide, the concrete items are: - Rectangular area in pixel grid, set explicitly / - Rendering operations are limited to this area / - All primitives that are completely or partially outside / are clipped at the viewport area (clipping). The object-level chain is camera/view volume, projection matrix, clip coordinates, normalized device coordinates, and viewport coordinates.
...
Professor-style explanation: The slide 'Framebuffer' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Context-bound raster with a resolution and structure / specified by the application / - Physical realization on the GPU / - Logical subdivision / - Color memory. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'OpenGL Framebuffers 1/3' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Typical OpenGL framebuffer configuration / 8bit Stencil Buffer / 32bit Depth Buffer / = 32bit Color Buffer. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'OpenGL Framebuffers 2/3' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - All buffers of a frame buffer have the same grid size / - Number of bitplanes per buffer can be configured individually / - Individual buffers are directly supported by the hardware / - Buffer identifier / - GL_COLOR_BUFFER_BIT. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'OpenGL Framebuffers 3/3' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Delete a buffer / glClear(GLbitfield mask); / - Set the initialization color for the color buffer / glClearColor(1.0,1.0,1.0,1.0); / glClear(GL_COLOR_BUFFER_BIT). The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Double Buffering' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Use of two color buffers to avoid flicker effects / - Image buildup takes place in the background (back buffer) / - Visible image is independent of this (front buffer) / Rendering / Process. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Pixel-based Rendering Operations' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Generally / - Read and write operations for various image data formats, / usually as an external library (e.g., DevIl) / - Conversions between image data types / - In OpenGL done with framebuffer objects. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Image Operations' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Rendering image data with OpenGL / Copy Pixel Data / Draw/Read / Pixel / Geometry. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Write Pixels into Framebuffer' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - glDrawPixels(x, y, width, height, format, type, pixels); / - Example / // 64x64 array with rgb components / GLubyte img[64][64][3]; / void draw() {. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Read Pixels from Framebuffer' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - glReadPixels(x, y, width, height, format, type, pixels); / - Example / GLubyte* img = 0; / int width, height; // canvas size / void snapshot() {. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'Formats & Data Types' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Format information for pixel array operations / - GL_RGB – red-green-blue values of the frame buffer (FB) / - GL_RED – red values o f the FB / - GL_ALPHA – transparency values of the FB / - GL_RGBA – red-Green-Blue-Alpha values of the FB. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
...
Professor-style explanation: The slide 'coordinate systems in the geometry stage' explains object-based rendering as a conversion chain. A model supplies geometric objects such as vertices or primitives. The geometry stage transforms and assembles those objects. Rasterization turns primitives into fragments, and fragment operations decide which fragments update pixels in the framebuffer. On the slide, the concrete items are: - Rendering pipeline consist of geometry stage and rasterization stage / - Geometric primitives (mostly triangles) are transformed through different / coordinate systems in the geometry stage / - Rasterization results in fragments, which are pixel predecessors / - Fragments have to endure a sequence of tests in the rasterization stage, in. The essential relation is that every stage changes the representation of the same scene information.
...
Professor-style explanation: The slide 'Guide: The Official Guide to Learning OpenGL' describes the OpenGL side of the rendering pipeline. OpenGL represents rendering through objects such as contexts, buffers, vertex arrays, shaders, textures, samplers, and framebuffers. At draw time, the currently bound objects and state determine what data reaches the GPU and how the pipeline processes it. On the slide, the concrete items are: - Text Books / - D. Shreiner, G. Sellers, J. Kessenich, B. Licea-Kane: OpenGL Programming / Guide: The Official Guide to Learning OpenGL / (8th Edition), Addison-Wesley 2013. / - P. Shirley, M. Ashikhmin, S. Marschner: Fundamentals of Computer. The concrete relation is between API state, GPU resource, shader input, and final rendering result.
